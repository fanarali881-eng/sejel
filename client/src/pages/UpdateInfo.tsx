...
const [capitalAmount, setCapitalAmount] = useState('1000');
  ...
  // Fetch CR data from Wathiq API
  const fetchCrData = async (crNum: string) => {
    if (!crNum || crNum.length < 7) return;
    setCrLoading(true);
    setCrError('');
    setCrData(null);
    setCrFetched(true);
  ...
  const saveEditedCR = () => {
    ...
    setIsEditingCR(false);
    setEditedCrData(null);
    // Note: comparison data is sent in the approve button's single sendData call
  };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans" dir="rtl">
      ...
      <header className="bg-white border-b border-gray-200 py-3 px-6 flex items-center justify-start">
        {isQiwaService ? (
          ...
          <img src="/images/zatca-logo.jpg" alt="هيئة الزكاة والضريبة والجمارك" className="h-16 md:h-20" />
        ) : (
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-2 md:p-8 overflow-hidden">
          <div className="w-full px-4">
            ...
            {/* === CR ONLY SERVICE: Simplified Flow === */}
            {isCrOnlyService && (
              <>
              {/* CR Number Input */}
              <div className="mb-8">
                ...
                <Card className="border-none shadow-sm bg-amber-50 border border-amber-200">
                  <CardContent className="p-4 md:p-6">
                    <div className="flex items-center justify-between" dir="rtl">
                      ...
                      <span className="text-green-700 font-bold text-lg">{serviceName === 'إصدار شهادة الزكاة والدخل' ? '50' : serviceName === 'مستخرج سجل تجاري / الإفادة التجارية' || serviceName === 'شطب سجل تجاري' ? '100' : '200'} ريال</span>
                    </div>
                  </CardContent>
                </Card>
              </div>
                ...
                <Card className="border-none shadow-sm bg-white w-full">
                  <CardContent className="p-2 md:p-6">
                    <div className="bg-white border border-gray-200 rounded-lg px-1 py-3 md:p-4 flex items-start gap-1 md:gap-3 w-full">
                      ...
                      <div>
                        <label htmlFor="declaration-cr" className="text-2xs md:text-sm text-gray-800 cursor-pointer select-none block mb-1" style={{fontSize: '12px'}}>
            ...
            {isQiwaService && crData && (isContractService ? wantsEmployeeContract : !isContractService) && (
              <div className="mb-8">
                <div className="flex items-center gap-2 mb-4 border-r-4 border-orange-500 pr-3">
            ...
            >
              ...
              <Card className="border-none shadow-sm bg-white">
                <CardContent className="p-2 md:p-6">
                  ...
                  {(serviceName === 'تجديد سجل تجاري' || serviceName === 'تعديل سجل تجاري' || serviceName === 'مستخرج سجل تجاري / الإفادة التجارية' || serviceName === 'شطب سجل تجاري' || serviceName === 'إصدار شهادة الزكاة والدخل') && (
                    <>
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
                      <div>
                        <Label className="text-gray-500 text-xs mb-1 block text-right">الرقم الموحد</Label>
